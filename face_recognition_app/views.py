        ...
        try:
            ...
            try:
                ...
                if not face_locations:
                    return JsonResponse({
                        'status': 'error',
                        'message': 'No face detected in the photo. Please ensure the face is clearly visible, not cropped, and the photo is well-lit.'
                    })

                # Get encodings
                face_encodings = face_recognition.face_encodings(rgb_image, face_locations, num_jitters=2)
                print(f"[DEBUG] Face Encodings Generated: {len(face_encodings)}")

                if not face_encodings:
                    return JsonResponse({
                        'status': 'error',
                        'message': 'Face detected but could not process encodings. Try using a clear, front-facing photo.'
                    })
...
def admin_portal(request):
    return render(request, 'face_recognition_app/admin_portal.html')
...
def view_all_students(request):
    students = Student.objects.all()
    return render(request, 'face_recognition_app/view_students.html', {
        'students': students
    })

# Sync attendance with external system or perform required logic
def sync_attendance(request):

    messages.success(request, "Attendance successfully synced!")
    return redirect('admin_portal')


def generate_daily_report(request):
    report_data = "Report data or content here"
    return render(request, 'face_recognition_app/daily_report.html', {
